...
> 一个基于 Skia 的跨平台自绘 UI 框架，覆盖桌面运行时、声明式 Markup 页面能力，以及 Android 宿主与原生运行时接线。

Tinalux 不包装系统原生控件，而是自己维护 Widget 树、布局、命中测试、绘制和主题系统。
当前仓库已经包含桌面窗口层、多渲染后端、UI 组件体系、声明式 Markup 管线，以及 Android 原生运行时与宿主层骨架。
...
- [`docs/UI框架能力清单.md`](./docs/UI框架能力清单.md)：UI 能力概览
- [`docs/Markup一页式速查.md`](./docs/Markup一页式速查.md)：Markup 快速上手与速查
